...
currentWeekStart.setDate(currentWeekStart.getDate() - (currentWeekStart.getDay() || 7) + 1); // Set to the start of the week (Monday)
...
function showWeek(weekStart) {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        // Apply grey background for Saturday and Sunday
        if (i === 5) {
...
function openModal(meeting) {
    ...
    document.getElementById('modalDate').textContent = new Date(meeting.date).toLocaleDateString('en-GB'); // Format as DD/MM/YYYY
...
// Functions to navigate through weeks
...
// Initial fetch and display of the current week
fetchMeetings();
