document.addEventListener('DOMContentLoaded', () => {
    ...
    //     try {
    //         // Fetch meetings from localStorage instead of a JSON file
    ...
    async function fetchMeetings() {
        try {
            // Fetch meetings from localStorage instead of a JSON file
            meetings = JSON.parse(localStorage.getItem('meetings')) || [];
            showWeek(currentWeekStart); // Show the week with meetings
        } catch (error) {
            console.error('Error fetching meetings:', error);
        }
    }
    ...
    function showWeek(weekStart) {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            // Filter and display meetings for the current day
    ...
    fetchMeetings(); // Fetch meetings when the page loads
});
